...
func New(sm subnet.Manager, extIface *backend.ExternalInterface) (backend.Backend, error) {
	be := &OVSBackend{
		...
		watches:  make(chan *network, 5),
	}

	var err error
	be.dev, err = newOVSDevice()
	if err != nil {
		return nil, err
	}
...
func (ovsb *OVSBackend) RegisterNetwork(ctx context.Context, netname string, config *subnet.Config) (backend.Network, error) {
	...
	net, err := newNetwork(netname, config, ovsb.extIface.Iface.MTU, lease, ovsb)
	if err != nil {
		return nil, err
	}
	ovsb.networks[net.name] = net
...
func (ovsb *OVSBackend) removeNetwork(network *network) {
	...
	delete(ovsb.networks, network.name)
}
...
func (ovsb *OVSBackend) Run(ctx context.Context) {
	...
	for k := range ovsb.networks {
		delete(ovsb.networks, k)
	}
