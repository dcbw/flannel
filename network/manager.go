...
fmt.Println(fmt.Sprintf("FOUND: %v", result.Snapshot))
		if err != nil {
			return nil, err
		}
...
func writeSubnetFile(path string, nw ip.IP4Net, ipMasq bool, sn *backend.SubnetDef) error {
...
func (m *Manager) Run(ctx context.Context) {
	...
	for _, net := range m.networks {
		go func(n *Network) {
			...
			sn := n.Init(ctx, m.extIface, m.iaddr, m.eaddr)
			if sn != nil {
				if m.isMultiNetwork() {
